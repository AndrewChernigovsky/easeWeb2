(() => {
	...
	// requireTask(`${gulpConfig.task.prettyHTML}`, `./${gulpConfig.folder.tasks}/`, {
	...
	gulp.task('pug', function (done) {
		gulp.src('templates/**/*.pug').pipe(pug()).pipe(pretty()).pipe(gulp.dest('./dist')).on('end', done);
	});
	...
	requireTask(`${gulpConfig.task.htmlformatter}`, `./${gulpConfig.folder.tasks}/`, {
		src: gulpConfig.folder.build,
		dest: gulpConfig.folder.build,
		checkProduction: true,
	});
	...
	requireTask(
		...
		`./${gulpConfig.folder.tasks}/`,
		{
			...
			tasks: {
				...
				prettyHTML: gulpConfig.task.prettyHTML,
				htmlformatter: gulpConfig.task.htmlformatter,
			},
		},
		false
	);
	...
	gulp.task(
		...
		gulp.series(
			...
			gulp.parallel(
				gulp.series(gulpConfig.task.fileIncludepug, gulpConfig.task.htmlformatter),
				gulp.series(gulpConfig.task.buildSass, gulpConfig.task.buildSassCustom, gulpConfig.task.buildStylesVendors),
				gulp.series(gulpConfig.task.buildCustomJs, gulpConfig.task.developJS, gulpConfig.task.buildJsVendors),
	...
	gulp.task('webp', gulp.series(gulpConfig.task.imageMin, gulpConfig.task.imageWebP));

	gulp.task('sprite', gulp.series(gulpConfig.task.svgSprite));
	gulp.task('php', gulp.series(gulpConfig.task.copyPHP));
})();
